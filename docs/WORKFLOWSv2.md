...
sequenceDiagram
    ...
    API->>DB: Update MetaInfo (View Mode + Publish)
...
public class TeamCollaborationService
{
    ...
    public async Task ExecuteTeamWorkflowAsync(Guid teamId, Guid userId)
    {
        ...
        var MetaInfo = await _context.MetaInfos.FindAsync(
        ...
        {
            ProjectId = MetaInfo.ProjectId,  // Updated FK relationship
...
public class AdhdFriendlyTaskService
{
    ...
    public async Task ExecuteFocusModeWorkflowAsync(Guid MetaInfoId)
    {
        ...
        var MetaInfo = await _context.MetaInfos
            ...
            .FirstOrDefaultAsync(ci => ci.Id == MetaInfoId);
        
        if (MetaInfo == null)
            return;
        
        // Step 2: Auto-save snapshot before major changes
        await _autosaveService.CreateSnapshotAsync(MetaInfo.Id, 
            SnapshotTypeEnum.AutoGenerated);
        
        // Step 3: Update content item in focus mode
        MetaInfo.Description = "Updated description in focus mode...";
        ...
        var snapshot = new ContentSnapshot
        {
            ...
            CreatedByUserId = _userContext.CurrentUser.Id,
            SnapshotDataJson = JsonSerializer.Serialize(MetaInfo.ToSnapshotModel()),
            CreatedAt = DateTime.UtcNow
        };
...
public class VersionControlWorkflowService
{
    ...
    public async Task ExecuteAutoSaveWorkflowAsync(Guid MetaInfoId)
    {
    ...
    public async Task ExecuteRollbackWorkflowAsync(Guid MetaInfoId, int targetVersion)
    {
        // Step 1: Find snapshot to rollback to
        var snapshot = await _context.ContentSnapshots
            .Include(s => s.CreatedByUser)
            .FirstOrDefaultAsync(s => 
                s.MetaInfoId == MetaInfoId && 
                s.SnapshotVersion == targetVersion);
        
        if (snapshot == null)
            throw new NotFoundException("Snapshot not found", MetaInfoId);
        
        // Step 2: Restore content from snapshot data
        var restoredItem = JsonSerializer.Deserialize<MetaInfo>(snapshot.SnapshotDataJson);
        
        // Step 3: Update existing item with restored data
        await _context.MetaInfos.UpdateAsync(restoredItem);
...
public async Task ExecuteRollbackDataRetrievalAsync(Guid MetaInfoId, int targetVersion)
{
    // Step 1: Find snapshot to rollback to
    var snapshot = await _context.ContentSnapshots
        .Include(s => s.CreatedByUser)
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Solo Writer** | Individual development with focus tools | MetaInfo, StoryOutline, **ProjectTask** | High | Ready for MVP | Renamed Task → ProjectTask ✅ |
