...
1. **Is this a detail model?** (Should have ContentItemId FK)
2. **Is this a junction table?** (Should NOT have universal metadata, only FKs)  
3. **Is this an independent entity?** (Should NOT have ContentItemId unless linking to content)
...
| # | Model | Type | Has ContentItemId? | Status | Notes |
|---|-------|------|-------------------|--------|-------|
| 1 | ContentItem.cs | Universal Container | N/A (Root) | ✅ OK | Removed type-specific collections already |
| 2 | StoryOutline.cs | Detail Model | ❌ NO | ❌ FAIL | NEEDS: Add ContentItemId FK |
| 3 | StorySequence.cs | Hierarchical Root | ❌ NO | ❌ FAIL | NEEDS: Add ContentItemId OR link via Project→ContentItems |
...
| 9 | Comment.cs | Junction Table | ✅ YES (to ContentItem/Task) | ✅ PASS | Pure junction, no universal metadata |
...
| 12 | ContentTags.cs | Junction Table | ✅ YES (to ContentItem) | ✅ PASS | Pure junction, no universal metadata |
...
| 17 | ProjectTask.cs | Detail Model | ❌ NO (Only ProjectId) | ❌ FAIL | NEEDS: Add ContentItemId FK |
...
| 19 | ReviewStatus.cs | Detail Model | ✅ YES | ✅ PASS | Already has ContentItemId FK |
...
- Already-fixed detail models (ReviewStatus with ContentItemId)

### **❌ NON-COMPLIANT (~35% of 57 = ~20 models):**
1. StoryOutline.cs - Missing ContentItemId ❌
2. StorySequence.cs - Missing ContentItemId ❌
3. ProjectTask.cs - Missing ContentItemId ❌
...
1. ✅ StoryOutline.cs - Add ContentItemId FK (narrative detail)
2. ✅ StorySequence.cs - Add ContentItemId FK (narrative root)
3. ✅ ProjectTask.cs - Add ContentItemId FK (task-detail model)
4. ❓ Verify and fix all dialogue models (Branch, Node, Background)
5. ❓ Verify StoryBeat for ContentItemId
6. ❓ Verify AssetLink for ContentItemId
7. ❓ Verify ContentSnapshot/VersionLog for ContentItemId
...
- Is it independent/system-wide (no ContentItemId needed)?

---

## ACTION PLAN

### **Phase 1: Add Missing ContentItemId FKs**
...
- **Performance:** Indexed joins on ContentItemId columns  
