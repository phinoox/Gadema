...
**Fix Required**: 
- Add `using GameDev.Core.Dtos.Response;` to ContentItemService.cs (line 3)
...
**Files needing fix**: All service files in `src/GameDev.Api/Services/`
```bash
# Pattern to add after "using GameDev.Core.Dtos;"
...
### Immediate Priority:
1. `src/GameDev.Api/Services/ContentItemService.cs` - Line 3 (add using), Lines 237, 321, 376 (fix return patterns)
2. `src/GameDev.Api/Services/TaskService.cs` - Lines 98-100, 127-139 (fix nullable handling)
3. `src/GameDev.Data/GameDbContext.cs` - Add Projects DbSet

### Secondary Priority:
4. All service files in `src/GameDev.Api/Services/` - Add EF Core using statement
...
dotnet ef migrations add InitialCreate --project GameDev.Data
dotnet run --project GameDev.Api
```
